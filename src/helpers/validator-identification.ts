...
export class ValidatorIdentification {
/**
 ...
 static esIdentificacionValida(identificacion: string) {
  ...
  static esRucPersonaNaturalValido(numeroRuc: string): boolean {
    const esIdentificacionValida = this.validacionesPrevias(
      numeroRuc,
      13,
      IdentificationTypeEnum.RUC_PERSONA_NATURAL
    );
  ...
  static esRucSociedadPrivadaValido(numeroRuc: string): boolean {
    const esIdentificacionValida = this.validacionesPrevias(
      numeroRuc,
      13,
      IdentificationTypeEnum.RUC_SOCIEDAD_PRIVADA
    );
    if (esIdentificacionValida) {
      const ultimoDigito: number = parseInt(numeroRuc.charAt(9), 10);
      return this.algoritmoVerificaIdentificacion(
  ...
  static esRucSociedadPublicaValido(numeroRuc: string): boolean {
    const esIdentificacionValida = this.validacionesPrevias(
      numeroRuc,
      13,
      IdentificationTypeEnum.RUC_SOCIEDAD_PUBLICA
    );
    if (esIdentificacionValida) {
  ...
  static validacionesPrevias(
    ...
    longitud: number,
    tipoIdentificacion: IdentificationTypeEnum
  ): boolean {
    if (IdentificationTypeEnum.CEDULA === tipoIdentificacion) {
      return (
        this.esNumeroIdentificacionValida(identificacion, longitud) &&
  ...
  static esTercerDigitoValido(
    ...
    if (tipoIdentificacion === IdentificationTypeEnum.CEDULA) {
      return this.esTercerDigitoCedulaValido(tercerDigito);
    }

    if (tipoIdentificacion === IdentificationTypeEnum.RUC_PERSONA_NATURAL) {
      return this.verificarTercerDigitoRucNatural(tercerDigito);
    }

    if (tipoIdentificacion === IdentificationTypeEnum.RUC_SOCIEDAD_PUBLICA) {
      return this.verificarTercerDigitoRucPublica(tercerDigito);
    }

    if (tipoIdentificacion === IdentificationTypeEnum.RUC_SOCIEDAD_PRIVADA) {
  ...
  static sumatoriaMultiplicacion(
    ...
    if (tipoIdentificacion === IdentificationTypeEnum.CEDULA) {
      ...
      tipoIdentificacion === IdentificationTypeEnum.RUC_PERSONA_NATURAL
    ) {
  ...
  static obtenerCoeficientes(
    ...
    if (
      tipoIdentificacion === IdentificationTypeEnum.CEDULA ||
      tipoIdentificacion === IdentificationTypeEnum.RUC_PERSONA_NATURAL
    ) {
      return [2, 1, 2, 1, 2, 1, 2, 1, 2];
    } else if (
      tipoIdentificacion === IdentificationTypeEnum.RUC_SOCIEDAD_PRIVADA
    ) {
      return [4, 3, 2, 7, 6, 5, 4, 3, 2];
    } else if (
      tipoIdentificacion === IdentificationTypeEnum.RUC_SOCIEDAD_PUBLICA
    ) {
